...
const {captchaCheck} = require(path.join(process.cwd(),'/middleware/captchaCheck.js'));
const {sendEmail} = require(path.join(process.cwd(),'/utils/emails.js'))
//Email Variables
...
router.post('/', captchaCheck, async (req, res) => {
    try {
        ...
        const info = await sendEmail("general", "creekvt", recipient, subject, messageContent)
